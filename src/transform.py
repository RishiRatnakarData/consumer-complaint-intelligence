...
def clean_complaints(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["has_relief"] = (
        df["company_response"]
        .str.contains("relief", case=False, na=False)
        .astype(int)
    )
    df["has_narrative"] = (
        df["complaint_what_happened"]
    ...
    return (
        df.sort_values(["date_received", "complaint_id"])
...
def quality_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    checks = [
        ...
        (
            "complaint_id_unique",
            df["complaint_id"].is_unique,
            int(df["complaint_id"].duplicated().sum()),
        ),
        (
            "date_received_complete",
            df["date_received"].notna().all(),
            int(df["date_received"].isna().sum()),
        ),
        (
            "product_complete",
            df["product"].ne("Unknown").all(),
            int(df["product"].eq("Unknown").sum()),
        ),
        (
            "is_timely_binary",
            df["is_timely"].dropna().isin([0, 1]).all(),
            int(
                (
                    ~df["is_timely"]
                    .dropna()
    ...
    return pd.DataFrame(
        checks,
        columns=["check_name", "passed", "observed"],
    )